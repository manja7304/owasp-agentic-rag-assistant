...
def main() -> int:
    ...
    banner("OWASP Agentic RAG Assistant")
    print(f"{DIM}Pattern: Agentic RAG · USE_MOCK_LLM=true (no Ollama/Docker required){RESET}\n")
    section("Agent API demo")
    from fastapi.testclient import TestClient
    from src.api.main import app

    client = TestClient(app)
    request_body = {'query': 'How to prevent SQL injection in APIs?'}
